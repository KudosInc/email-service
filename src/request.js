...
const get = ({ headers, uri }) => new Promise((resolve, reject) => {
  const req = http.request(uri, { headers, method: 'GET' }, (res) => callback(res, resolve, reject));
  req.on('error', reject);
  req.end();
});

const post = ({ headers, uri, body }) => new Promise((resolve, reject) => {
  const req = http.request(uri, { headers, method: 'POST' }, (res) => callback(res, resolve, reject));
  req.on('error', reject);
